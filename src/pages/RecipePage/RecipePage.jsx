...
const RecipePage = () => {
  ...
  useEffect(() => {
    ...
    const fetchPopularRecipes = async () => {
      setIsLoading(true);
      try {
        const { data } = await apiService.get('/recipes/popular');
        setPopularRecipes(data);
      } catch (err) {
        setError(err.message);
        toast.error(`Failed to load popular recipes: ${err.message}`);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  return (
    <div className={styles.recipePage}>
      <PathInfo recipeTitle="Battenberg Cake" />
      <RecipeInfo
        recipeId={id}
        testRecipeData={testRecipeData}
        testIngredients={testIngredients}
        testOwner={testOwner}
      />
      <PopularRecipes recipes={[]} /> {/* Replace with fetched popular recipes */}
    </div>
  );
};
