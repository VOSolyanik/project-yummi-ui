...
const CustomDropdown = ({
  ...
  return (
    <div className={`${css.dropdown} ${className}`} ref={dropdownRef}>
      ...
      >
        <span className={css.selectedText}>{selectedOption ? selectedOption.label : placeholder}</span>
        <Icon name="arrow-down" size={18} className={`${css.chevron} ${isOpen ? css.rotated : ''}`} />
      </button>
          ...
          {options.map(option => (
            <button
              ...
              className={clsx(css.option, {
                [css.selected]: selectedOption?.value === option.value,
                [css.all]: option.value === ''
              })}
